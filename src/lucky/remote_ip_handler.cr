...
# to the value of the first IP in the `HTTP_X_FORWARDED_FOR` header
# or fallback to the default `remote_address`.
#
...
class Lucky::RemoteIpHandler
  ...
  private def fetch_remote_ip(context : HTTP::Server::Context) : String?
    ...
    x_forwarded = request.headers["HTTP_X_FORWARDED_FOR"]?.try(&.split(',').first?)
